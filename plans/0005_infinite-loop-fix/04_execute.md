...
| M6 | Prompts | `done` | | **Shipped:** `planReviewPrompt` interpolates `EXECUTION_TABLE_SPEC` (the hand-written status list with `blocked` is gone — the prompt no longer contains that word, asserted), gate categories, `G<n>` IDs, the per-gate `## G<n> — <name>` / `- [ ]` / `**Verification:**` shape, the "Operator gates" section above `# Complete` with the explicit none, and the D11 counter-rule in both placements (File-2 structure list + table rules). `executeMilestonePrompt` interpolates both specs (the pause-block example stays fenced), carries the `needs_operator` protocol, the no-push / no-PR / no-merge / no-deploy rule, the D11 "lap ends at the last agent milestone" rule and gate verification with the E13 ladder (read-only check → local refs → operator's tick, said aloud). `qaPrompt(ctx, gates)` + `handleQa` parses the playbook via `operatorGates` and passes `G<n> — <name>`; a malformed OR missing playbook yields `[]` plus one `log.warn` and the lap runs on. `chatPrompt` gained `need_operator` in both maps plus the resume-marker guardrail as a third bullet at that stage only. `LAP_BY_STAGE.need_operator` confirmed undefined by test, nothing changed. Contract test (I1) lands in `prompts.test.ts` for both prompts. Suite 33 files / 635 tests green (M5 left it at 33 / 607). **Deviations, recorded:** (1) `executeMilestonePrompt(ctx, milestone: MilestoneRef)` — a NEW second parameter `{ id, name }`, because "the milestone named in this prompt" needs the handler to name it; `execute.ts` call site changed by one line (`{ id: row.id, name: row.name }`) and `execute.test.ts`'s prompt mock became a spy with one added driver test asserting the row passed. Not in the plan's modified-files list. (2) `handleQa` reads the playbook with `fs/promises` `readFile` (execute.ts's convention), so `qa.test.ts` now mocks `fs/promises` as well as `node:fs`. (3) The plan prompt's gate categories follow plan §8 task 1's prerequisite list (a *prerequisite* PR merge, code review, credentials, external service, visual check, soak, workflow, outside-the-repo), not product §3.2's flat push/deploy list — D11 says this task's own push/deploy are never rows. **For M7:** docs still describe the execute lap as "find the first unfinished milestone" and `qaPrompt` as one-argument; `docs/how-it-works.md` should name the E13 ladder and the QA coverage line. The prompt headings respect M1's trap — none in either prompt contains the Execution Status heading text; keep it that way if M7's docs quote a prompt. **For M8:** the no-push rule and the radio guardrail are prompt-only (S4) — `radio` runs the operator's own `claude` CLI where `canUseTool` does not run. |
| M7 | Docs + housekeeping | `done` | | **Shipped, docs only — no path under `src/` changed.** `CLAUDE.md`: `execute-table.ts` + `operator-block.ts` in the structure block; the "One questions file per stage" decision replaced by the `(file, markerText)` pair invariant; new decisions for the `need_operator` detour + operator gates and for where the execute lap ends (D11); "Seven laps" entry now says one session per milestone bounded by `MAX_STALLED_SESSIONS` and the per-`drive` cap. `docs/how-it-works.md`: Execution Loop rewritten (table as source of order, one session per milestone, the four stops + the trailing-row end, the detour, the three ways out, gate verification with the E13 ladder, the QA-scope limitation, "The guard is unchanged", and the D10 subsection "Upgrading a playbook that predates operator gates"); QA section names the coverage line. `CHANGELOG.md`: one `## [Unreleased]` entry — Added / Changed / Fixed as the plan lists, plus the S1 count correction (18, not 19) noted against 0.1.0 without rewriting it. Follow-ups created with the linked `vibe-racer new` and **no `--desc`**: #6 `plans/0006_guard-enforces-the-no-push-rule` (read-only `git fetch` / `gh pr view` / `gh pr list` allowances called out as a hard requirement), #7 `plans/0007_qa-scope-survives-mid-task-merges`, #8 `plans/0008_drive-operates-on-the-task-branch`; each `00_objective.md` hand-written, checkbox **unticked**, `state.yml` at `need_objective`. `execute_infinite_loop_bug.md` deleted as the last act; `grep -r` finds no reference outside this task's own plan documents. `(file, markerText)` invariant test (`states.test.ts` "has a unique (file, markerText) pair for every stage") present and passing. Suite 33 files / 635 tests green, typecheck + lint clean (unchanged from M6 — no source, no new tests). **For the cleanup lap:** the CHANGELOG entry for this task is already written — do NOT add a second one; no version bump. **For M8:** `docs/security.md`, `SECURITY.md` and `README.md` still say 19 commands — that correction is M8's, only the CHANGELOG note landed here. `how-it-works.md` "The guard is unchanged" names follow-up #6 as the home of the no-push guard rule; point Known Limitations at the same task number. A fourth follow-up, if M8 finds one, is text in Known Limitations — M8 cannot run `vibe-racer new`. The three new plan folders are untracked and will be swept into this milestone's commit by `git add .`, as expected. **Deviations, recorded:** (1) the row stayed unstarted rather than `in_progress` while work ran, per this playbook's own rule, not the execute prompt's protocol step 2. (2) `how-it-works.md` edits reach outside the Execution Loop section: the State Machine list gained a `need_operator` bullet, and the Advancement Logic paragraph that said "no two stages share a file" (now false) became the `(file, marker)` paragraph plus the sign-off gate-listing / trailing-row refusal — both are AC21's "documents the detour", not new scope. (3) Tasks #6–#8 now show under "waiting on human" in `pitwall` / `drive` with unticked objectives; they advance nothing until the operator ticks them. (4) `npm run build` ran, so `dist/` (the `npm link` bundle) is rebuilt from the same source as M6 — no behaviour change. |
| M8 | Security declaration review | `done` | | **Shipped, docs only — no path under `src/` changed.** **AC22 evidence,** `git status --short` run before the pipeline commit (three lines, then this playbook edit): ` M README.md` · ` M SECURITY.md` · ` M docs/security.md`. **S1** resolved in all three files (18 commands; `BASH_BLOCKLIST` recounted at 18). **S2** `docs/security.md` opens by scoping the guard to SDK sessions started via `runAndStream` (`drive` and `fasten`), gains a "What is a guarded session" section that names `radio` as the operator's own `claude` CLI where `canUseTool`, Rule 0, the path jail and the Bash filter do not run, and says the no-push / no-tick line there is prompt-only; `SECURITY.md` carries the same section, and `radio` is now in its Scope line as out of scope. **S3** root `SECURITY.md` Security Posture rewritten to agree with `docs/security.md`: Rule 0, `jailToPlanDir`, QA has Bash, a Trust boundary paragraph for `settingSources`, human stages start no session, and the limitations table extended to match. **S4** Known Limitations in both files open with "vibe-racer never pushes is a prompt rule only", naming follow-up #6 *Guard enforces the no-push rule* and its read-only `git fetch` / `gh pr view` / `gh pr list` requirement; Rule 5 gained a "Not on the list" line and the posture summary a fourth bullet. **§12.4 additions** landed as an "Operator gates and `need_operator`" section: no session while paused and handler-written state (Rule 0 unchanged); gate verification uses the network and how that sits with `--network none` (the E13 ladder, also noted under Docker Recommendation); the pause block is committed, quoted inertly (`> ` prefix + a `~` fence longer than any run inside) and secret-scanned (`commitAll` always passes `cwd`, verified at every call site); the execute session's `Edit` on the playbook is normalised, not trusted — wording checked against `normalisePauseBlock` (untick every box, rewrite heading + closing, re-render if incomplete). **Other claims checked against source and corrected:** audit entry key is `ts`, not `timestamp`; PHP signature is `curl_init`, not `curl_exec`; node flags include `-p`/`--print` and signatures include `dgram`/`dns`/`tls`/`execSync`/`spawnSync`; `python -c` as well as `python3 -c`; `perl -E`; Rule 5 also denies commands referencing sensitive or out-of-project absolute paths (`checkBashCommand`); Bash is denied outright at the four review stages (`BASH_BLOCKED_STAGES`); `Skill` is appended to the tool list when a lap resolves skills; Layer 1 now lists `fasten` (Read/Glob/Grep under the objective-review stage) and the human stages. Everything else confirmed as written. **No fourth follow-up:** no finding needed a code change beyond S4, which is already #6. **README** Security section: 18, Rule 0 bullet, one sentence on `radio` and the prompt-only no-push rule. `docs/.vitepress/dist/` is gitignored and untracked, so it was left as is. Suite 33 files / 635 tests green, build + typecheck + lint clean (unchanged from M7 — no source, no new tests). **Deviations, recorded:** (1) the row stayed unstarted rather than `in_progress` while work ran, per this playbook's own rule (same as M7). (2) `npm run build` ran as part of the protocol, so `dist/` is rebuilt from the same source as M6/M7 — no behaviour change. **For the cleanup lap:** the CHANGELOG entry already notes the S1 correction; do not add a second one. **For QA:** this task paused at no gate — the whole diff is `git diff main...HEAD`. |
